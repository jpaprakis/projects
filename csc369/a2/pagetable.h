...
//This is a struct for the lru function
struct lru_list {
	int frame; //Holds the physical frame number corresponding to coremap
	struct lru_list *next;
};

//adding a struct to hold the second level page table of the opt function
typedef struct { 
	struct lru_list *occurences; //holds a linked list containing the occurences of the adress in this node
	int is_init; // check if node initialized or not
	struct lru_list *end_ptr; //points to the end of the occurences list so that we don't have to loop through to find where to insert
} opt_second_lvl;  

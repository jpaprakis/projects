...
void update_heap()
{
	//update max_heap to hold the address of the current value of the program break
...
void touch_after_malloc(long id, int index, char *ptr, int size)
{
	...
	//debug_print("[%li]: malloc block %d addr %p size %d touching complete\n",
	  //          id, index, ptr, size);
	void * temp = ptr;
	temp -= sizeof(struct allocatedlist);
	struct allocatedlist *alloc = temp;
	printf("Malloc header is %d size and %d magic\n", alloc->size, alloc->magic);
	if (alloc->size < size+sizeof(struct allocatedlist))
	{
		printf ("MALLOC size error!!!\n");
		exit(2);
	}
	else if (alloc->magic != -88232467)
	{
		printf ("MALLOC magic error!!\n");
		exit(2);
	}
}
	...
	for (i = 0; i < ops; i++) {
		...
		case MALLOC:
			...
			if ((ptr < start_heap) || (ptr + tr.ops[i].size > max_heap)) {
				printf("max heap is %p\n", max_heap);
				printf("ptr is %p\n", ptr);
				//printf("max heap size is %s and ptr + tr.ops[1].size is %s\n", max_heap, d);

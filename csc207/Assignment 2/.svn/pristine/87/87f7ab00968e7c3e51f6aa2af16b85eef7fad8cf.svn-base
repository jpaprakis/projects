...
public class Input {
	...
	public static Object enteredCommand(String commandName) {
		...
		if (commandParts.get(0).equals("pwd")) {
			...
			if (inBounds == 1) {
				...
				//redirect contents
				File.fileRedirection(Print.printDirContents(
						Directory.workingDirectory.directoryLocation()), 
						commandParts.get(commandParts.size()-1),
						commandParts.get(commandParts.size()-2));
				...
				if (inBounds == 2) {
					...
					System.out.println(AllCommands.recursivePrintDirContents(
							Directory.workingDirectory.directoryLocation()));
				}
				else if (inBounds == 4 && redirect) {
					...
					for (String part : commandParts.subList(
							...
							if (checkPath(part, false)) {
									System.out.println(
											AllCommands.
											recursivePrintDirContents(part));
						}
					}
				}
				//if it is redirect
					...
					for (String part : commandParts.subList(
						...
						if (checkPath(part, false)) {
							//redirect
							File.fileRedirection(AllCommands
									.recursivePrintDirContents(part), 
									commandParts.get(commandParts.size()-1),
									commandParts.get(commandParts.size()-2));
						}
				...
				for (String path : commandParts.subList(1,
					...
					if (checkPath(path, false)) {
						File.fileRedirection(Print.printDirContents(path), 
								commandParts.get(commandParts.size()-1),
								commandParts.get(commandParts.size()-2));
					}
				}
		...
		// The following commands require at least 1 argument
			...
			if (inBounds == 1) {
				...
				if (inBounds == 4) {
					if (parsePath(commandParts.get(1))[1] instanceof File) {
						if ((redirectInput(commandParts.get(
								commandParts.size()-2),
							...
							File.fileRedirection(Print.printFileContents(
									(File) parsePath(commandParts.get(1))[1]), 
									commandParts.get(commandParts.size()-1),
									commandParts.get(commandParts.size()-2));
						}
					}
					else {
						AllCommands.raiseError("Item is not a file.");
					}
				}
				else if (inBounds == 2) {
					if (parsePath(commandParts.get(1))[1] instanceof File) {
					...
					AllCommands.raiseError("Invalid number of arguments");
				}
			}
		}
			...
			if (inBounds == 2 || redirect) {
				...
				if (checkPath(commandParts.get(1), false)) {
					if ((dirPair[0] instanceof Directory)) {
						AllCommands.changeWorkingDirectory(
								(Directory) dirPair[0]);
					}
					if (dirPair[1] instanceof Directory) {
						AllCommands.changeWorkingDirectory(
								(Directory) dirPair[1]);
					}
				}
			...
			if (commandParts.size() >= 3) {
				if ((redirectInput(commandParts.get(
						commandParts.size()-2), Arrays.asList(1), 1)) 
						&& inBounds >= 4) {
					//can have: mkdir dir1, dir2, etc >> file
					commandParts = commandParts.subList(0, commandParts
							.size()-2);		
				}
			}
			...
			if (inBounds >= 2) {
				for (String dirPath : commandParts.subList(1,
					...
					if (checkPath(dirPath, true)) {
						...
						{
							if (!checkInput(dirPath)) {
								new Directory(splitLastPath(dirPath),
										(Directory) parsePath(dirPath)[0]);
							}
						} 
						else {
							AllCommands.raiseError("An item with the same "
			...
			if (commandParts.size() >= 3) {
				if ((redirectInput(commandParts.get(
						commandParts.size()-2), Arrays.asList(4, 5)
						, inBounds))) {
					//can have: rm path >> file
					//can have: rm -f path >> file
					//trim it to get rid of the >> file part if true
					commandParts = commandParts.subList(0, commandParts
							.size()-2);
				}
			...
			if(inBounds == 2){
				...
				if(commandParts.get(1).equals("-f") || commandParts.get(1)
						.equals("-F")){
					if(checkPath(commandParts.get(2), false)){
						Directory.removeDir(true, parsePath(commandParts
								.get(2))[1]);
					}
				}
				else{
		...
		else if (commandParts.get(0).equals("grep")) {
			if (inBounds < 3) {
				AllCommands.raiseError("Invalid number of arguments.");
			}
			else {
				if (commandParts.get(1).equals("-r") || 
						commandParts.get(1).equals("-R")) {
					for (String path : commandParts.subList(
							3, commandParts.size())) {
						if (checkPath(path, false)) {
							if (parsePath(path)[1] instanceof Directory) {
								System.out.println(Print.printGrep(
										AllCommands.grep(commandParts.get(2).
												substring(1, commandParts.
														get(2).length()-1), 
														parsePath(path)[1])));
							}
							else if (path.equals("/")) {
								...
								AllCommands.raiseError("Can only use grep " +
					...
					for (String path : commandParts.subList(
							2, commandParts.size())) {
						if (checkPath(path, false)) {
							if (parsePath(path)[1] instanceof File) {
								System.out.println(Print.printGrep(
										AllCommands.grep(commandParts.get(1).
												substring(1, commandParts.
														get(1).length()-1), 
														parsePath(path)[1])));
							}
							else {
								AllCommands.raiseError("Can only use grep non" +
										" recursively with files.");
							}
						}
					}
				}
	...
	/**
	 * Checks to see if the input is redirecting to a file. Returns true if it
	 * is redirecting, and if the amount of arguments specified are correct
	 * for that particular command's redirect call.
	 * 
	 * @param commandPart
	...
	private static boolean redirectInput (String commandPart, 
		...
		if (commandPart.equals(">") || commandPart.equals(">>")) {

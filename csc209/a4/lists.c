...
char *list_calendars(Calendar *cal_list) {
    ...
    //Looping through the list to find the size of each item
    //and adding it to user_size, +2 for the network newline
    while (current != NULL) {
        user_size+=strlen(current->name) +2;
        current = current->next;
    }

    //adding 1 to user_size to account for final null terminator
    user_size ++;

    //Declaring str_list, where the strings will be stored to be returned by 
    //the function
    char *str_list;
    if ((str_list = malloc(user_size)) == NULL)
    {
        perror("malloc");
        exit(1);
    }
    ...
    //Loop through and add all the calendar strings to the buffer str_list, with
    //a network newline after each
    while (current != NULL) {
        writesize += snprintf(str_list+writesize, user_size-writesize, "%s\r\n", 
            current->name);
        current = current->next;
    }
...
char* list_users(User *user_list) {
    ...
    //Declaring str_list, where the strings will be stored to be returned by the 
    //function
    char *str_list;

    /*
    I will never overwrite the space allocated for str_list because the
    integer user_size keeps track of exactly how much space this will 
    require. It keeps track of this in the while loop above, by adding the
    length of each user string plus 1 (to account for the new line character).
    An extra 1 will be added to the end of user_size once it loops through fully
    to account for the final new line character and null terminator. The length 
    in user_size will vary depending on the amount and size of the strings in
    the user list. 

    The way that snprintf works is that it will write the string I desire to
    the buffer str_list, including the network newline, and add its own null 
    terminator to the end of that. The next time round, since I am telling it to
    write the next string at str_list+writesize, it will overwrite the previous
    null terminator and keep going, until it gets to the very end. This is why
    it is necessary to add +1 to user_size above at the very end, for that very
    last null terminator that doesn't get overwritten.
    */
    if ((str_list = malloc(user_size)) == NULL)
    {
        perror("malloc");
    ...
    //Looping through and appending the user strings to buffer str_list
    while (current != NULL) {
        writesize += snprintf(str_list+writesize, user_size-writesize, "%s\r\n", 
            current->name);
        current = current->next;
...
char *list_events(Calendar *cal_list, char *cal_name) {
    ...
    //If the calendar can't be found, return an error message stating so
    if ((this_cal = find_calendar(cal_list, cal_name)) == NULL) {
        
        if ((str_list = malloc(38)) == NULL)
        {
            perror("malloc");
            exit(1);
        }

        snprintf(str_list, 30, "Calendar could not be found\r\n");
        return(str_list);
    }

    //Otherwise the calendar was found, continue down here

    int event_size = 0;
    Event *cur = this_cal->events;

    //Loop through to get the size of each event
    while (cur != NULL) {
        event_size += strlen(cur->description);

        //Add an extra 28: time_t will always be length of 26, plus 2 for the 
        //network newline
        event_size += 28;
        cur = cur->next;
    }

    //One last +1 to size for the final null terminator
    event_size ++;

    //add in space to account for the first line when outputting the events to 
    //the user - fixed 23 characters including the network new line, plus the 
    //calendar name
    event_size += 23+strlen(this_cal->name);

    //Mallocing for str_list where strings will be stored to be returned by the 
    //function
    if ((str_list = malloc(event_size)) == NULL)
    {
        perror("malloc");
        exit(1);
    }
    ...
    //Do an initial 1-time snprintf to str_list with just the "events for calendar"
    //line which will only happen at the top
    writesize += snprintf(str_list+writesize, event_size-writesize, 
        "Events for calendar %s:\r\n",this_cal->name);

    cur = this_cal->events;
    
    //Now iterating through and writing to buffer str_list each event
    while (cur != NULL) {

        //Setting ctime to time_temp so that I can strip the newline character 
        //to properly send a network newline
        char time_temp [26];
        strncpy(time_temp, ctime(&(cur->time)), 26);
        
        //Stripping the newline character in time_temp so that I can return 
        //a network newline instead
        *(time_temp + 21) = '\0';

        writesize += snprintf(str_list+writesize, event_size-writesize, 
            "%s: %s\r\n",cur->description, time_temp);
        cur = cur->next;
    }

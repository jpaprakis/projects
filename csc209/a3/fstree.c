...
//Declaring the mark_tree function so that it can call mark_and_sweep periodically
void mark_tree(void *tree_head);
...
Fstree *create_node(char *name) {
    ...
    //First blocking any signals so that they don't interrupt when create_node
    //is being called
    ...
    //Now actually performing create_node actions
...
Link *create_link(Fstree *fp) {
    ...
    //First blocking any signals so that they don't interrupt when create_link
    //is being called
    /* Initialize signal set */
    sigset_t sigs;
    sigemptyset(&sigs);
    ...
    //Now performing the actual create_link actions
    Link *lp = gc_malloc(sizeof(Link));
    lp->fptr = fp;
    //printf("new link's fptr name is %s\n", lp->fptr->name);
    lp->next = NULL;
...
void add_node(char *path, char *name) {
    //First blocking any signals so that they don't interrupt when add_node
    //is being called
    /* Initialize signal set */
    sigset_t sigs;
    sigemptyset(&sigs);
    ...
    //Now performing actual add_node actions
    ...
    //printf("new link's next is %s's links\n", fp->name);
    fp->links = new_link;
    //printf("%s's links are pointing to the new link obj", fp->name);
...
void do_transactions(char *transfile) {
    ...
    while((lp = fgets(line, MAX_LINE, fp)) != NULL) {
        ...
        //Call mark_and_sweep every 10 times the program goes through the while
        if (count % 10 == 0)
        {
